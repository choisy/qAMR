...
cb_val <- seq2(0, 25)
k_mu_val <- k_mu(cb_val, pm, mu)

plotl(NA, xlim = c(0, 4), ylim = c(0, 3), xaxs = "i", yaxs = "i")

rect(0, 0, 5, 4, col = colS)
polygon(c(0, cb0, 0), c(3, k0 / mu_val, 3), col = colI)
#polygon(c(0, min(cb1), cb1, 0), c(3, 0, k1 / mu_val, 3), col = colR)
#polygon(c(4, rev(cb_val), 4), c(3, rev(k_mu_val), 3), col = "grey")

lines2(cb0, k0 / mu_val, col = 3)
lines2(cb1, k1 / mu_val, col = 2)
# abline(v = 1 / c(1 - p1 / pm, 1 - p0 / pm))
abline(v = 1 / c(1 - p1 / pm, 1 - p02 / pm), col = 2:3)
lines(cb_val, k_mu_val, lwd = 4)
```

```{r}
plot(NA, xlim = c(0, 4), ylim = c(0, 3), xaxs = "i", yaxs = "i")

rect(0, 0, 5, 4, col = colS)
polygon(c(0, 0, cb0, 0), c(3, 0, k0 / mu_val, 3), col = 2)
#polygon(c(0, min(cb1), cb1, 0), c(3, 0, k1 / mu_val, 3), col = colR)
#polygon(c(4, rev(cb_val), 4), c(3, rev(k_mu_val), 3), col = "grey")
...
```{r}
h_S <- function(cb, p0, pm, mu) {
  mu / (1 / (1 - p0 / pm) - cb)
}
```

```{r}
h_R <- function(cb, k0, k1, p1, pm, mu) {
  mu * k0 / ((1 / (1 - p1 / pm) - cb) * k1)
}
```

```{r}
h_S0 <- function(cb, p0, pm, mu) {
  mu * (((1 - p0 / pm)^-1) - cb)^-1
}
```

```{r}
h_R0 <- function(cb, k0, k1, p1, pm, mu) {
  (mu * k0 * (((1 - p1 / pm)^-1) - cb)^-1) / k1
}
```

```{r}
cb_val <- seq2(0, 20)
h_R_val <- h_R0(cb_val, k0, k1, p1, pm, mu)
plotl(cb_val, h_S_val)
```
