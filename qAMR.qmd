...
abline(h = .432)
rect2(-1, -1, th1, 1, col = colS)
rect2(th2, -1, 11, 1, col = colR)
lines2(x, p(x - 5, .45, .25, .1), col = 4)
lines2(x, p(x - 8.5, .45, .25, .1))
```
...
```{r}
x <- seq2(-5, 5)
plotl(x, dnorm(x, 0, 1), lwd = 8)
```

```{r}
x <- seq2(-5, 5)
plot(x, dnorm(x, 0, 1), type = "n")
polygon3(x, dnorm(x, 0, 1))
```
...
lines2(x, p(x, 10, 5, 3), lwd = 8, col = 4)
```
...
```{r}
plot_R0c <- function(k0 = 10, k1 = .1, pm = 10, p0 = 9.5, p1 = 4.75, mu = .95,
                    x = seq2(-1, 2)) {
  R0_drug <- R0(x, k0, k1, pm, p0, p1, mu)
  R0_nodrug <- R0(x, 0, 0, pm, p0, p1, mu)
  xStar <- R_evol_level(k0, k1, p1, p0, pm, mu)
...
```{r}
subplots <- function(sim0, sim1, plot2 = TRUE) {
  opar <- par(mfrow = c(3, 1), plt = c(.15, .98, .24, .95), cex = 1)
...
y1 <- sim0$pop_size
y2 <- sim1$pop_size
plotllog1(sim0$time, y1, yaxs = "i", ylim = c(0, max(c(y1, y2))), col = 2,
          xlab = "time", ylab = "population size")
lines2(sim1$time, y2, col = 3)
```
