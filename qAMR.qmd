...
```{r}
convolution <- function(x, y) rowSums(x %*% t(y))
```

```{r}
simulator <- function(size0, x0, sigma0, pr, pm, p0, p1, k0, k1,
                      alpha, mu, eps, x, time) {
  dt <- diff(time)
  dx <- mean(diff(x))
  ps <- p(x, pm, p0, p1)
  ...
  bact_pop <- bact_popNA <- as.matrix(as.data.frame(bact_pop))
...
eps <- .01
#eps <- .005
system.time(out <- simulator(size0, x0, sigma0, pr, pm, p0, p1, k0, k1, alpha, mu, eps, x, time))
```
