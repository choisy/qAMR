...
```{r}
simulator <- function(size0, x0, sigma0, pm, p0, p1, k0, k1, alpha, mu, eps, x, time) {
  dt <- diff(time)
  dx <- mean(diff(x))
  ps <- p(x, pm, p0, p1)
  ks <- k(x, k0, k1)
  ...
  if (sigma0 > 0) {
    distribution <- dnorm(x, x0, sigma0)
  ...
  if (eps > 0) {
    A <- 2 * eps^2
    B <- sqrt(pi * A)
    J <- map(x, ~ exp(-(.x - x)^2 / A) / B)
    update_bact_pop <- function(bact_pop, pop_size) {
      L <- map_dbl(J, ~ sum(.x * ps * bact_pop)) * dx / (1 + pop_size)^alpha
  ...
  for (dt_val in dt) {
    mr_bact_pop <- update_bact_pop(mr_bact_pop, mr_pop_size)
...
out <- simulator(size0, x0, sigma0, pm, p0, p1, k0, k1, alpha, mu, eps, x, time)
```
...
```{r}
with(out2D, image(time, x, bact_pop, bty = "o", col = plot3D::jet.col(),
...
Plotting the bacterial population size, the mean resistance, and the variance of
resistance as a function of time:

```{r fig.height = 3 * 5 / 2, fig.width = 7 / 2}
opar <- par(mfrow = c(3, 1), plt = c(.15, .98, .24, .95), cex = 1)
with(out2, plotllog(time, log10(pop_size), log = 1:2,
                    xlab = "time", ylab = "log10(population size)"))
plotllog(out2$time, mean_resistance(out2), log = 1,
         xlab = "time", ylab = "mean level of resistance")
plotllog(out2$time, resistance_variance(out2), log = 1,
...
The efficacy of the drug relative to the natural death rate of the bacteria (including
everything (immune system, etc...) but the drug):
...
mutation <- simulator(size0, x0, sigma0, pm, p0, p1, k0, k1, alpha, mu, eps, x, time) |> 
  log_transform()
no_mutation <- simulator(size0, x0, sigma0, pm, p0, p1, k0, k1, alpha, mu, 0, x, time) |> 
  log_transform()
...
y1 <- log10(mutation$pop_size)
y2 <- log10(no_mutation$pop_size)
plotllog(x, y1, log = 1:2, ylim = range(c(y1, y2)),
         xlab = "time", ylab = "log10(population size)")
lines2(x, y2)

y1 <- mean_resistance(mutation)
y2 <- mean_resistance(no_mutation)
plotllog(x, y1, log = 1, ylim = range(c(y1, y2)),
         xlab = "time", ylab = "mean level of resistance")
lines2(x, y2)

y1 <- resistance_variance(mutation)
y2 <- resistance_variance(no_mutation)
plotllog(x, y1, log = 1, ylim = range(c(y1, y2)),
         xlab = "time", ylab = "variance of resistance")
lines2(x, y2)
...
sim0 <- simulator(size0, x0, sigma0, pm, p0, p1, k0, k1, alpha, mu, 0, x, time) |> 
  log_transform()
sim1 <- simulator(size0, x0, 0, pm, p0, p1, k0, k1, alpha, mu, 0, x, time) |> 
  log_transform()

plotllog(sim0$time, sim0$pop_size, log = 1, ylim = c(0, 4),
         xlab = "time", ylab = "population size")
lines2(sim1$time, sim1$pop_size)
```
